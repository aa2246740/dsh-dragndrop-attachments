...
The product video is a **real recording** of DeepSeek Harness Web UI (for example `http://127.0.0.1:3080`): drag the fixtures below onto the page, then stitch the clips.

`docs/demo/out/plugin-demo.mp4` and `plugin-demo.gif` are **placeholders**. Do not ship them as the demo.

Target cut: **15–30 s**, landscape 16:9, H.264 MP4. Durations below are a budget, not a hold-every-beat rule.
...
Record the DeepSeek Harness **Web UI** (not slides). Start the Host, open the session in the browser, start recording, do the action, hold the result, stop. Name the file `NN-id.mov` or `NN-id.mp4` (example: `01-drag-image.mov`).
...
- **ID:** `02-drag-folder`
- **Duration:** 3 s
- **Action:** Drag the `sample-folder` directory from Finder (or the OS file manager) onto the page.
...
- **ID:** `03-paste`
- **Duration:** 2 s
- **Action:** Copy `paste-note.md` (or a screenshot) to the clipboard. Paste near the composer.
...
```sh
./assemble.sh              # stitch raw/*.mov|mp4 in shot order
./assemble.sh --dry-run    # print FOUND/MISSING only
```

If any `raw/NN-id.mov|mp4` is missing, the script prints the missing IDs, does not encode, and exits 0.

When every clip is present it writes:

- `docs/demo/out/plugin-demo.mp4` — X-ready H.264, 16:9
- `docs/demo/out/plugin-demo.gif` — looping README embed

ffmpeg errors or a half-written MP4 are failures. Missing raw clips are not.
